    ...
    doInit : function(component, event) {
        ...
        if(dealModelWrapper && dealModelWrapper != null && dealModelWrapper.data != null && dealModelWrapper.data.ET_MARGIN_DETAILS != null){
            ...
            for(var i = 0; i < products.length; i++){
                console.log('=======PRODUCT_ID====== ',products[i].PRODUCT_ID+ ' '+products[i].PRODUCT_CATEGORY);
        ...
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
                ...
                var lstItemDetails = [];
                var lstItemDetails1 = [];
                var itemdetailscomponent = component.get('v.itemdetailscomponent');
                console.log('itemdetailscomponent: '+itemdetailscomponent);
                ...
                for(const dwProductLine of productMap.keys()) {
                    ...
                    for(const productID of productMap.get(dwProductLine)){
                        ...
                        if(margindetailMap.has(productID)){
                            ...
                            var products1 = margindetailMap.get(productID);
                            console.log('products: '+productID);
                            for(var i=0 ; i<products.length;i++){
                                
                                if(itemdetailscomponent === true){
                                    if(i > 0){
                                        products1[0].QUANTITY = products1[0].QUANTITY + products1[i].QUANTITY; 
                                    }
                                }else{
                            ...
                            if(itemdetailscomponent === true){
                                //products1[0].NEW_BOOK_VAL_OMNICELL = products1[0].NEW_BOOK_VAL_OMNICELL * products1[0].QUANTITY; //IBA-4873 Sourav
                            	lstProductNew1.push(products1[0]);
                    ...
                    if(lstProductNew.length > 0){
                         lstItemDetails.push({ title : dwProductLine, lstProduct :lstProductNew});
                    }
                    
                    if(itemdetailscomponent === true){
                    	if(lstProductNew1.length > 0){
                            lstItemDetails1.push({ title : dwProductLine, lstProduct :lstProductNew1});
                        }
                    }
                }
                ...
                for(var i = 0; i < dealModelWrapper.data.ET_MARGIN_DETAILS.length; i++){
                    ...
                    var products1 = dealModelWrapper.data.ET_MARGIN_DETAILS[i];
                    var products2 = dealModelWrapper.data.ET_MARGIN_DETAILS[i];
                    if(itemdetailscomponent === true){
                        console.log(products2.NEW_BOOK_VAL_OMNICELL+' ===ttt1=== '+products2.QUANTITY);
                        lstProductNew1.push(products1);                   
                        
                    }else{
                        console.log(products2.NEW_BOOK_VAL_OMNICELL+' ===tt2=== '+products2.QUANTITY);
                        products2.NEW_BOOK_VAL_OMNICELL = products2.NEW_BOOK_VAL_OMNICELL * products2.QUANTITY;
                        lstProductNew.push(products2);  
                    }
                }
                if(lstProductNew.length > 0){
                    lstItemDetails.push({ title : 'Other', lstProduct :lstProductNew});
                }
                if(lstProductNew1.length > 0 && itemdetailscomponent === true){
                    lstItemDetails1.push({ title : 'Other', lstProduct :lstProductNew1});
                }
                if(itemdetailscomponent === true){
                    component.set('v.lstItemDetails1', lstItemDetails1);
                }
                ...
                for(var item = 0; item < lstItemDetails.length; item++){
                    ...
                    var prodSet = new Set();//IBA-4873 Sourav
                    for(var ii=0;ii < listprdcts.length;ii++){
                        if(prodSet.has(listprdcts[ii].PRODUCT_ID)){ //IBA-4873 Sourav start
                            continue;
                        }
                        prodSet.add(listprdcts[ii].PRODUCT_ID) //IBA-4873 Sourav End
                        if(listprdcts[ii].NON_DISCOUNTABLE === 'X')
                            nondiscamount = nondiscamount+(listprdcts[ii].NEW_BOOK_VAL_OMNICELL);//IBA-4873 Sourav
                        else
                            discamount = discamount+(listprdcts[ii].NEW_BOOK_VAL_OMNICELL);//IBA-4873 Sourav
                        totamount = totamount+(listprdcts[ii].NEW_BOOK_VAL_OMNICELL);//IBA-4873 Sourav                        
                    }
